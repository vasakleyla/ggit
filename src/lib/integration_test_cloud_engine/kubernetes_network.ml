...
module Node = struct
  ...
  let graphql_uri node =
    let host = sprintf "%s.graphql.test.o1test.net" node.config.testnet_name in
    let path = sprintf "/%s/graphql" node.app_id in
    Uri.make ~scheme:"http" ~host ~path ~port:80 ()

  let graphql_client ~logger node =
  ...
  let get_metrics ~logger t =
    graphql_client ~logger t |> Test_graphql.get_metrics

  let get_best_chain ?max_length ~logger t =
    graphql_client ~logger t |> Test_graphql.get_best_chain ?max_length

  let get_account_data ~logger t ~account_id =
    graphql_client ~logger t |> Test_graphql.get_account_data ~account_id

  let get_account_permissions ~logger t ~account_id =
    graphql_client ~logger t |> Test_graphql.get_account_permissions ~account_id
  ...
  *)
  ...
  let send_zkapp_batch ~logger (t : t)
    ...
    graphql_client ~logger t |> Test_graphql.send_zkapp_batch ~zkapp_commands
  ...
  let send_test_payments ~repeat_count ~repeat_delay_ms ~logger t ~senders
    ...
    graphql_client ~logger t
    |> Test_graphql.send_test_payments ~repeat_count ~repeat_delay_ms ~senders
         ~receiver_pub_key ~amount ~fee
  ...
  let set_snark_worker ~logger t ~new_snark_pub_key =
    graphql_client ~logger t |> Test_graphql.set_snark_worker ~new_snark_pub_key
